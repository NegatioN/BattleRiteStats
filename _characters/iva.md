---
builds:
  threes:
  - damage: '7.1'
    disable: '1.3'
    energy: '3.4'
    num: 23
    protection: '1.1'
    skills:
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: pink
      description: Increases Oil factor by 10 and duration by 1.5.
      icon: a001d096a99f9a84da3e8b1056dc0535
      name: Gunge
      type: control
    - color: red
      description: Tazer consumes Oil to inflict Burning Oil.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Igniting Spark
      type: offense
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '65.2'
  - damage: '4.4'
    disable: '0.7'
    energy: '2.7'
    num: 17
    protection: '0.7'
    skills:
    - color: red
      description: Rocket X-67 deals 4 bonus damage.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Boom!
      type: offense
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: teal
      description: Zap absorbs 12 more damage on yourself and 16 more damage on other
        allies.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Durable Shield
      type: support
    - color: pink
      description: Increases Oil factor by 10 and duration by 1.5.
      icon: a001d096a99f9a84da3e8b1056dc0535
      name: Gunge
      type: control
    - color: yellow
      description: ^2Jet Pack^- increases movement speed by 30 for 2.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Rocket Boosters
      type: mobility
    winrate: '29.4'
  - damage: '5.9'
    disable: '1.0'
    energy: '2.7'
    num: 15
    protection: '0.9'
    skills:
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: blue
      description: Tazer removes positive effects from nearby enemies and negative
        effects from nearby allies on Spell Block interrupts.
      icon: c9cd772b84da5454e9888a19fa249483
      name: EMP
      type: utility
    - color: pink
      description: Increases Oil factor by 10 and duration by 1.5.
      icon: a001d096a99f9a84da3e8b1056dc0535
      name: Gunge
      type: control
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '53.3'
  - damage: '6.6'
    disable: '1.0'
    energy: '3.1'
    num: 14
    protection: '0.6'
    skills:
    - color: yellow
      description: Zap knocks nearby enemies back and grants 20 Fading Haste for 2.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Blast Shield
      type: mobility
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: red
      description: Tazer consumes Oil to inflict Burning Oil.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Igniting Spark
      type: offense
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: yellow
      description: ^2Jet Pack^- increases movement speed by 30 for 2.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Rocket Boosters
      type: mobility
    winrate: '50.0'
  - damage: '7.0'
    disable: '1.0'
    energy: '3.7'
    num: 13
    protection: '0.8'
    skills:
    - color: yellow
      description: Zap knocks nearby enemies back and grants 20 Fading Haste for 2.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Blast Shield
      type: mobility
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: blue
      description: Tazer removes positive effects from nearby enemies and negative
        effects from nearby allies on Spell Block interrupts.
      icon: c9cd772b84da5454e9888a19fa249483
      name: EMP
      type: utility
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '69.2'
  - damage: '5.6'
    disable: '1.3'
    energy: '2.8'
    num: 12
    protection: '0.7'
    skills:
    - color: red
      description: Machine Gun inflicts Oil to enemies in a frontal cone when activated
        and launches a Rocket X-67 when firing the last bullet.
      icon: 7dc5f22ab9743154eb85f40db35cf33b
      name: All Systems Operational
      type: offense
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: pink
      description: Increases Oil factor by 10 and duration by 1.5.
      icon: a001d096a99f9a84da3e8b1056dc0535
      name: Gunge
      type: control
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    winrate: '41.7'
  - damage: '6.1'
    disable: '1.3'
    energy: '4.0'
    num: 11
    protection: '0.7'
    skills:
    - color: yellow
      description: Zap knocks nearby enemies back and grants 20 Fading Haste for 2.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Blast Shield
      type: mobility
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: blue
      description: Tazer removes positive effects from nearby enemies and negative
        effects from nearby allies on Spell Block interrupts.
      icon: c9cd772b84da5454e9888a19fa249483
      name: EMP
      type: utility
    - color: yellow
      description: ^2Jet Pack^- increases movement speed by 30 for 2.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Rocket Boosters
      type: mobility
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    winrate: '63.6'
  - damage: '5.3'
    disable: '0.6'
    energy: '1.9'
    num: 11
    protection: '1.2'
    skills:
    - color: red
      description: Machine Gun inflicts Oil to enemies in a frontal cone when activated
        and launches a Rocket X-67 when firing the last bullet.
      icon: 7dc5f22ab9743154eb85f40db35cf33b
      name: All Systems Operational
      type: offense
    - color: red
      description: Rocket X-67 deals 4 bonus damage.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Boom!
      type: offense
    - color: teal
      description: Zap absorbs 12 more damage on yourself and 16 more damage on other
        allies.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Durable Shield
      type: support
    - color: blue
      description: Tazer removes positive effects from nearby enemies and negative
        effects from nearby allies on Spell Block interrupts.
      icon: c9cd772b84da5454e9888a19fa249483
      name: EMP
      type: utility
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    winrate: '36.4'
  - damage: '5.4'
    disable: '1.0'
    energy: '2.4'
    num: 10
    protection: '0.6'
    skills:
    - color: pink
      description: Rocket X-67 hits inflict Fading Snare for 1.5 and the explosion
        inflicts Fading Snare for 0.7.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Heavy Rocket
      type: control
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: red
      description: Tazer consumes Oil to inflict Burning Oil.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Igniting Spark
      type: offense
    - color: red
      description: Rocket X-67 gains 1 ability charge but cooldown increases by 1.5.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Stockpile
      type: offense
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '40.0'
  - damage: '6.2'
    disable: '1.0'
    energy: '2.5'
    num: 9
    protection: '0.9'
    skills:
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: pink
      description: Increases Oil factor by 10 and duration by 1.5.
      icon: a001d096a99f9a84da3e8b1056dc0535
      name: Gunge
      type: control
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '77.8'
  twos:
  - damage: '4.8'
    disable: '0.7'
    energy: '3.2'
    num: 11
    protection: '0.6'
    skills:
    - color: red
      description: Rocket X-67 deals 4 bonus damage.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Boom!
      type: offense
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: teal
      description: Zap absorbs 12 more damage on yourself and 16 more damage on other
        allies.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Durable Shield
      type: support
    - color: pink
      description: Increases Oil factor by 10 and duration by 1.5.
      icon: a001d096a99f9a84da3e8b1056dc0535
      name: Gunge
      type: control
    - color: yellow
      description: ^2Jet Pack^- increases movement speed by 30 for 2.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Rocket Boosters
      type: mobility
    winrate: '36.4'
  - damage: '5.6'
    disable: '1.5'
    energy: '3.8'
    num: 9
    protection: '0.5'
    skills:
    - color: yellow
      description: Zap knocks nearby enemies back and grants 20 Fading Haste for 2.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Blast Shield
      type: mobility
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: red
      description: Rocket X-67 gains 1 ability charge but cooldown increases by 1.5.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Stockpile
      type: offense
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '88.9'
  - damage: '7.5'
    disable: '1.0'
    energy: '3.8'
    num: 8
    protection: '0.6'
    skills:
    - color: red
      description: Rocket X-67 deals 4 bonus damage.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Boom!
      type: offense
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: red
      description: Rocket X-67 gains 1 ability charge but cooldown increases by 1.5.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Stockpile
      type: offense
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '87.5'
  - damage: '7.5'
    disable: '1.3'
    energy: '3.8'
    num: 8
    protection: '1.1'
    skills:
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: red
      description: Tazer consumes Oil to inflict Burning Oil.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Igniting Spark
      type: offense
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '62.5'
  - damage: '6.0'
    disable: '1.1'
    energy: '3.5'
    num: 8
    protection: '1.1'
    skills:
    - color: yellow
      description: Zap knocks nearby enemies back and grants 20 Fading Haste for 2.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Blast Shield
      type: mobility
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: red
      description: Rocket X-67 gains 1 ability charge but cooldown increases by 1.5.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Stockpile
      type: offense
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '75.0'
  - damage: '7.7'
    disable: '1.0'
    energy: '4.8'
    num: 6
    protection: '1.0'
    skills:
    - color: red
      description: Rocket X-67 deals 4 bonus damage.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Boom!
      type: offense
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: teal
      description: Zap absorbs 12 more damage on yourself and 16 more damage on other
        allies.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Durable Shield
      type: support
    - color: yellow
      description: ^2Jet Pack^- increases movement speed by 30 for 2.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Rocket Boosters
      type: mobility
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '66.7'
  - damage: '8.0'
    disable: '1.2'
    energy: '4.3'
    num: 3
    protection: '0.6'
    skills:
    - color: yellow
      description: Zap knocks nearby enemies back and grants 20 Fading Haste for 2.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Blast Shield
      type: mobility
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: teal
      description: Zap absorbs 12 more damage on yourself and 16 more damage on other
        allies.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Durable Shield
      type: support
    - color: yellow
      description: ^2Jet Pack^- increases movement speed by 30 for 2.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Rocket Boosters
      type: mobility
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    winrate: '100.0'
  - damage: '6.3'
    disable: '1.0'
    energy: '3.5'
    num: 2
    protection: '0.9'
    skills:
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: yellow
      description: ^2Jet Pack^- increases movement speed by 30 for 2.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Rocket Boosters
      type: mobility
    - color: red
      description: Rocket X-67 gains 1 ability charge but cooldown increases by 1.5.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Stockpile
      type: offense
    - color: pink
      description: Tractor Beam inflicts Stun for 0.7 to enemies caught in the beam
        when the effect ends and cooldown reduces by 3.
      icon: fadd8bcda43b2f54aa66dd5d12795a53
      name: Tractor Pulse
      type: control
    winrate: '50.0'
  - damage: '5.1'
    disable: '1.1'
    energy: '3.4'
    num: 2
    protection: '0.8'
    skills:
    - color: red
      description: Rocket X-67 deals 4 bonus damage.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Boom!
      type: offense
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: pink
      description: Rocket X-67 hits inflict Fading Snare for 1.5 and the explosion
        inflicts Fading Snare for 0.7.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Heavy Rocket
      type: control
    - color: grey
      description: Jet Pack causes your next Boomstick to inflict Oil.
      icon: 7442381b626556c4e8193c48031d01ef
      name: Oil Blaster
      type: mixed
    - color: blue
      description: Recast Jet Pack to fly back to your starting location.
      icon: 7442381b626556c4e8193c48031d01ef
      name: U-Turn
      type: utility
    winrate: '50.0'
  - damage: '7.6'
    disable: '0.8'
    energy: '4.3'
    num: 2
    protection: '1.3'
    skills:
    - color: red
      description: Rocket X-67 deals 4 bonus damage.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Boom!
      type: offense
    - color: pink
      description: Tazer hits bounce once towards a nearby enemy.
      icon: c9cd772b84da5454e9888a19fa249483
      name: Conductor
      type: control
    - color: teal
      description: Zap absorbs 12 more damage on yourself and 16 more damage on other
        allies.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Durable Shield
      type: support
    - color: red
      description: Zap increases your Boomstick attack and reload speed by 120 while
        you're affected by the Shield.
      icon: 9274b3f40ab2b4048b5cde1e9164901e
      name: Hyperspeed
      type: offense
    - color: red
      description: Rocket X-67 gains 1 ability charge but cooldown increases by 1.5.
      icon: d80cae4dd7699264fb3c37675b59f263
      name: Stockpile
      type: offense
    winrate: '0.0'
compos:
  threes:
  - hero_names:
    - Poloma
    - Bakko
    - Iva
    heros:
    - icon: 9872f8cd16d7327418b09294cfa319e6
      name: Poloma
      title: poloma
    - icon: 6154d6c0556261343844bf7caebb67a7
      name: Bakko
      title: bakko
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 10
    winrate: 0.7
    wins: 7
  - hero_names:
    - Poloma
    - Iva
    - Raigon
    heros:
    - icon: 9872f8cd16d7327418b09294cfa319e6
      name: Poloma
      title: poloma
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 6c2252900d15cb344a5240f30d3ba705
      name: Raigon
      title: raigon
    num: 10
    winrate: 0.8
    wins: 8
  - hero_names:
    - Rook
    - Iva
    - Pearl
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 427116cdee46bad4cbaa99d7063e0832
      name: Pearl
      title: pearl
    num: 9
    winrate: 0.6666666666666666
    wins: 6
  - hero_names:
    - Iva
    - Sirius
    - Raigon
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    - icon: 6c2252900d15cb344a5240f30d3ba705
      name: Raigon
      title: raigon
    num: 8
    winrate: 0.5
    wins: 4
  - hero_names:
    - Rook
    - Poloma
    - Iva
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 9872f8cd16d7327418b09294cfa319e6
      name: Poloma
      title: poloma
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 8
    winrate: 0.875
    wins: 7
  - hero_names:
    - Iva
    - Croak
    - Sirius
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: d3038956f9767d74bbbce0659b07474d
      name: Croak
      title: croak
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    num: 7
    winrate: 0.42857142857142855
    wins: 3
  - hero_names:
    - Thorn
    - Iva
    - Pearl
    heros:
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 427116cdee46bad4cbaa99d7063e0832
      name: Pearl
      title: pearl
    num: 6
    winrate: 0.5
    wins: 3
  - hero_names:
    - Pestilus
    - Thorn
    - Iva
    heros:
    - icon: a7e0c7a6f0fa2ef49bf684950de34567
      name: Pestilus
      title: pestilus
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 6
    winrate: 0.5
    wins: 3
  - hero_names:
    - Thorn
    - Iva
    - Sirius
    heros:
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    num: 6
    winrate: 0.16666666666666666
    wins: 1
  - hero_names:
    - Iva
    - Bakko
    - Lucie
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 6154d6c0556261343844bf7caebb67a7
      name: Bakko
      title: bakko
    - icon: e5554b8a15ffc1a499a3807bb3a37b56
      name: Lucie
      title: lucie
    num: 6
    winrate: 0.3333333333333333
    wins: 2
  twos:
  - hero_names:
    - Iva
    - Blossom
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 6f50396e26efe6c4bbad274e2c90b8a4
      name: Blossom
      title: blossom
    num: 17
    winrate: 0.7058823529411765
    wins: 12
  - hero_names:
    - Iva
    - Pearl
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 427116cdee46bad4cbaa99d7063e0832
      name: Pearl
      title: pearl
    num: 16
    winrate: 0.5625
    wins: 9
  - hero_names:
    - Poloma
    - Iva
    heros:
    - icon: 9872f8cd16d7327418b09294cfa319e6
      name: Poloma
      title: poloma
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 13
    winrate: 0.8461538461538461
    wins: 11
  - hero_names:
    - Pestilus
    - Iva
    heros:
    - icon: a7e0c7a6f0fa2ef49bf684950de34567
      name: Pestilus
      title: pestilus
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 8
    winrate: 0.625
    wins: 5
  - hero_names:
    - Iva
    - Sirius
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    num: 7
    winrate: 0.2857142857142857
    wins: 2
  - hero_names:
    - Iva
    - Raigon
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 6c2252900d15cb344a5240f30d3ba705
      name: Raigon
      title: raigon
    num: 6
    winrate: 0.3333333333333333
    wins: 2
  - hero_names:
    - Rook
    - Iva
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 6
    winrate: 0.3333333333333333
    wins: 2
  - hero_names:
    - Iva
    - Ezmo
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 3af1fdc53169d044b939cce7ffb561a1
      name: Ezmo
      title: ezmo
    num: 5
    winrate: 0.2
    wins: 1
  - hero_names:
    - Iva
    - Bakko
    heros:
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    - icon: 6154d6c0556261343844bf7caebb67a7
      name: Bakko
      title: bakko
    num: 4
    winrate: 0.25
    wins: 1
  - hero_names:
    - Thorn
    - Iva
    heros:
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 4
    winrate: 0.75
    wins: 3
description: A scavenger from the outer realms. Iva has engineered her own arsenal
  of weapons. Firing crazy rockets or unleashing a storm of bullets is her way of
  greeting her opponents in the Arena.
icon: decba44679fbdae4bb7d0d1cb6a97682
layout: character
name: iva
num:
  threes: 445
  twos: 112
title: Iva
url: characters/iva.html
winrate:
  threes: '48.09'
  twos: '53.57'
...
