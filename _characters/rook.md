---
builds:
  threes:
  - damage: '6.3'
    disable: '1.2'
    energy: '3.0'
    num: 210
    protection: '1.9'
    skills:
    - color: red
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Armor
        Break</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crumble
      type: offense
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '56.2'
  - damage: '6.3'
    disable: '1.1'
    energy: '2.8'
    num: 139
    protection: '1.4'
    skills:
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '54.0'
  - damage: '5.8'
    disable: '1.0'
    energy: '2.7'
    num: 97
    protection: '1.7'
    skills:
    - color: red
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Armor
        Break</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crumble
      type: offense
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '54.6'
  - damage: '6.3'
    disable: '1.1'
    energy: '2.7'
    num: 82
    protection: '1.6'
    skills:
    - color: red
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Armor
        Break</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crumble
      type: offense
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '69.5'
  - damage: '5.8'
    disable: '1.1'
    energy: '2.8'
    num: 80
    protection: '1.1'
    skills:
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    winrate: '41.2'
  - damage: '6.3'
    disable: '1.4'
    energy: '3.4'
    num: 66
    protection: '1.8'
    skills:
    - color: pink
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Fading
        Snare</color> for <color=#64E4EFFF>1.5s</color>. Also reduces its cooldown
        by <color=#64E4EFFF>1.5s</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crag
      type: control
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    winrate: '56.1'
  - damage: '5.6'
    disable: '1.1'
    energy: '2.8'
    num: 63
    protection: '1.2'
    skills:
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: yellow
      description: <color=#00f4ffff>Pummel</color> hits reduce cooldown of <color=#00f4ffff>Rush</color>
        by <color=#64E4EFFF>0.4s</color>.
      icon: 037a9d14b1a74c249a0bb90af0237a7f
      name: Pummel Rush
      type: mobility
    winrate: '55.6'
  - damage: '5.4'
    disable: '1.0'
    energy: '2.7'
    num: 59
    protection: '1.4'
    skills:
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '49.2'
  - damage: '4.6'
    disable: '0.9'
    energy: '3.3'
    num: 59
    protection: '0.5'
    skills:
    - color: pink
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Fading
        Snare</color> for <color=#64E4EFFF>1.5s</color>. Also reduces its cooldown
        by <color=#64E4EFFF>1.5s</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crag
      type: control
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: <color=#00f4ffff>Pummel</color> hits reduce cooldown of <color=#00f4ffff>Rush</color>
        by <color=#64E4EFFF>0.4s</color>.
      icon: 037a9d14b1a74c249a0bb90af0237a7f
      name: Pummel Rush
      type: mobility
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    winrate: '39.0'
  - damage: '5.8'
    disable: '1.1'
    energy: '3.1'
    num: 45
    protection: '1.5'
    skills:
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '53.3'
  twos:
  - damage: '6.0'
    disable: '1.1'
    energy: '2.9'
    num: 38
    protection: '1.4'
    skills:
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    winrate: '47.4'
  - damage: '5.0'
    disable: '0.9'
    energy: '2.5'
    num: 28
    protection: '1.5'
    skills:
    - color: red
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Armor
        Break</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crumble
      type: offense
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '42.9'
  - damage: '6.5'
    disable: '1.2'
    energy: '2.7'
    num: 20
    protection: '1.7'
    skills:
    - color: red
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Armor
        Break</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crumble
      type: offense
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '60.0'
  - damage: '5.8'
    disable: '0.9'
    energy: '2.7'
    num: 17
    protection: '1.5'
    skills:
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '64.7'
  - damage: '6.1'
    disable: '1.2'
    energy: '2.9'
    num: 17
    protection: '1.6'
    skills:
    - color: red
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Armor
        Break</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crumble
      type: offense
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '76.5'
  - damage: '5.6'
    disable: '0.9'
    energy: '2.5'
    num: 16
    protection: '1.4'
    skills:
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '50.0'
  - damage: '7.2'
    disable: '1.3'
    energy: '2.9'
    num: 12
    protection: '0.8'
    skills:
    - color: red
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Armor
        Break</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crumble
      type: offense
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    - color: teal
      description: Enemies affected by <color=#00f4ffff>Armor Break</color> deal <color=#64E4EFFF>25%</color>
        less damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Weapon Break
      type: support
    winrate: '75.0'
  - damage: '3.7'
    disable: '0.7'
    energy: '15.9'
    num: 9
    protection: '0.7'
    skills:
    - color: pink
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Fading
        Snare</color> for <color=#64E4EFFF>1.5s</color>. Also reduces its cooldown
        by <color=#64E4EFFF>1.5s</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crag
      type: control
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: green
      description: Decreases damage taken during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>25%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Giant
      type: survival
    - color: yellow
      description: <color=#00f4ffff>Pummel</color> hits reduce cooldown of <color=#00f4ffff>Rush</color>
        by <color=#64E4EFFF>0.4s</color>.
      icon: 037a9d14b1a74c249a0bb90af0237a7f
      name: Pummel Rush
      type: mobility
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    winrate: '11.1'
  - damage: '5.4'
    disable: '1.2'
    energy: '2.8'
    num: 9
    protection: '1.3'
    skills:
    - color: pink
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Fading
        Snare</color> for <color=#64E4EFFF>1.5s</color>. Also reduces its cooldown
        by <color=#64E4EFFF>1.5s</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crag
      type: control
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: yellow
      description: <color=#00f4ffff>Pummel</color> hits reduce cooldown of <color=#00f4ffff>Rush</color>
        by <color=#64E4EFFF>0.4s</color>.
      icon: 037a9d14b1a74c249a0bb90af0237a7f
      name: Pummel Rush
      type: mobility
    winrate: '22.2'
  - damage: '5.8'
    disable: '1.4'
    energy: '2.7'
    num: 8
    protection: '0.8'
    skills:
    - color: pink
      description: <color=#00f4ffff>Boulder Toss</color> inflicts <color=#00f4ffff>Fading
        Snare</color> for <color=#64E4EFFF>1.5s</color>. Also reduces its cooldown
        by <color=#64E4EFFF>1.5s</color>.
      icon: ec50e13a62a38194186bf0763d8ae951
      name: Crag
      type: control
    - color: yellow
      description: Reduces the cooldown of <color=#00f4ffff>Rush</color> by <color=#64E4EFFF>1s</color>.
        Hitting an enemy further reduces cooldown by <color=#64E4EFFF>1s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Endurance
      type: mobility
    - color: red
      description: <color=#00f4ffff>Rush</color> hits cause you to gain <color=#00f4ffff>Berserk</color>
        for <color=#64E4EFFF>1.5s</color>.
      icon: 0cfff8a481a64fd499bfaeb3da1cbca5
      name: Frenzy
      type: offense
    - color: yellow
      description: Increases movement speed bonus during <color=#00f4ffff>Berserk</color>
        by <color=#64E4EFFF>30%</color>.
      icon: df78f943fca96314181fdf1827f825db
      name: Madness
      type: mobility
    - color: red
      description: <color=#00f4ffff>Crushing Blow</color> deals <color=#64E4EFFF>4</color>
        bonus damage.
      icon: bc4e2e34829b8af4c92bb6a8f570fc96
      name: Rook Smash
      type: offense
    winrate: '62.5'
compos:
  threes:
  - hero_names:
    - Rook
    - Blossom
    - Varesh
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 6f50396e26efe6c4bbad274e2c90b8a4
      name: Blossom
      title: blossom
    - icon: f1cc53a29c58089478f6438589d9b32d
      name: Varesh
      title: varesh
    num: 36
    winrate: 0.6666666666666666
    wins: 24
  - hero_names:
    - Rook
    - Blossom
    - Alysia
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 6f50396e26efe6c4bbad274e2c90b8a4
      name: Blossom
      title: blossom
    - icon: 50404d271ae2a4f4f8b52de966688dc1
      name: Alysia
      title: alysia
    num: 32
    winrate: 0.375
    wins: 12
  - hero_names:
    - Rook
    - Oldur
    - Jade
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: b0d66b8b9f228724093b8a3e66c3fef6
      name: Oldur
      title: oldur
    - icon: 38b75268719e2b640a6d62250e102ffa
      name: Jade
      title: jade
    num: 32
    winrate: 0.5
    wins: 16
  - hero_names:
    - Rook
    - Sirius
    - Jade
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    - icon: 38b75268719e2b640a6d62250e102ffa
      name: Jade
      title: jade
    num: 31
    winrate: 0.5806451612903226
    wins: 18
  - hero_names:
    - Rook
    - Sirius
    - Varesh
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    - icon: f1cc53a29c58089478f6438589d9b32d
      name: Varesh
      title: varesh
    num: 28
    winrate: 0.42857142857142855
    wins: 12
  - hero_names:
    - Rook
    - Jumong
    - Blossom
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 42a665b9c285a1243a201c8d533980e5
      name: Jumong
      title: jumong
    - icon: 6f50396e26efe6c4bbad274e2c90b8a4
      name: Blossom
      title: blossom
    num: 26
    winrate: 0.5769230769230769
    wins: 15
  - hero_names:
    - Rook
    - Poloma
    - Varesh
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 9872f8cd16d7327418b09294cfa319e6
      name: Poloma
      title: poloma
    - icon: f1cc53a29c58089478f6438589d9b32d
      name: Varesh
      title: varesh
    num: 26
    winrate: 0.6538461538461539
    wins: 17
  - hero_names:
    - Rook
    - Oldur
    - Varesh
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: b0d66b8b9f228724093b8a3e66c3fef6
      name: Oldur
      title: oldur
    - icon: f1cc53a29c58089478f6438589d9b32d
      name: Varesh
      title: varesh
    num: 26
    winrate: 0.46153846153846156
    wins: 12
  - hero_names:
    - Rook
    - Alysia
    - Sirius
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 50404d271ae2a4f4f8b52de966688dc1
      name: Alysia
      title: alysia
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    num: 25
    winrate: 0.56
    wins: 14
  - hero_names:
    - Rook
    - Poloma
    - Ashka
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 9872f8cd16d7327418b09294cfa319e6
      name: Poloma
      title: poloma
    - icon: 02fbe1fea9e22054a9ab1b0c5007dbae
      name: Ashka
      title: ashka
    num: 25
    winrate: 0.52
    wins: 13
  twos:
  - hero_names:
    - Rook
    - Oldur
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: b0d66b8b9f228724093b8a3e66c3fef6
      name: Oldur
      title: oldur
    num: 28
    winrate: 0.5357142857142857
    wins: 15
  - hero_names:
    - Rook
    - Sirius
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: aa93dc83288cb954aa3a97aabe8320dd
      name: Sirius
      title: sirius
    num: 24
    winrate: 0.4166666666666667
    wins: 10
  - hero_names:
    - Rook
    - Ruh Kaan
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: bb37e5d448c511e47b808987d0d4f9e8
      name: Ruh Kaan
      title: ruh-kaan
    num: 23
    winrate: 0.6086956521739131
    wins: 14
  - hero_names:
    - Rook
    - Blossom
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 6f50396e26efe6c4bbad274e2c90b8a4
      name: Blossom
      title: blossom
    num: 21
    winrate: 0.7142857142857143
    wins: 15
  - hero_names:
    - Rook
    - Jade
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 38b75268719e2b640a6d62250e102ffa
      name: Jade
      title: jade
    num: 17
    winrate: 0.35294117647058826
    wins: 6
  - hero_names:
    - Rook
    - Zander
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 9c4186a6214f12448a7091710b0cde6c
      name: Zander
      title: zander
    num: 17
    winrate: 0.4117647058823529
    wins: 7
  - hero_names:
    - Rook
    - Croak
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: d3038956f9767d74bbbce0659b07474d
      name: Croak
      title: croak
    num: 16
    winrate: 0.25
    wins: 4
  - hero_names:
    - Pestilus
    - Rook
    heros:
    - icon: a7e0c7a6f0fa2ef49bf684950de34567
      name: Pestilus
      title: pestilus
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    num: 16
    winrate: 0.625
    wins: 10
  - hero_names:
    - Rook
    - Bakko
    heros:
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 6154d6c0556261343844bf7caebb67a7
      name: Bakko
      title: bakko
    num: 12
    winrate: 0.25
    wins: 3
  - hero_names:
    - Ulric
    - Rook
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    num: 12
    winrate: 0.4166666666666667
    wins: 5
description: A mighty chieftain rushing into battle with a giant hammer in one hand
  and his dinner in the other. Rook likes to smash his enemies, with boulders, with
  hammers and even with his body. Once he goes berserk he turns into an unstoppable
  force.
icon: e5d5913527b842b41aed666d27d14231
layout: character
name: rook
num:
  threes: 1732
  twos: 312
title: Rook
url: characters/rook.html
winrate:
  threes: '50.81'
  twos: '46.15'
...
