---
builds:
  threes:
  - damage: '3.8'
    disable: '0.6'
    energy: '3.0'
    num: 71
    protection: '6.8'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: grey
      description: Holy Light heals you for 8 health when another ally affected by
        Favor is healed.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Divine Abundance
      type: mixed
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '39.4'
  - damage: '5.7'
    disable: '0.8'
    energy: '3.9'
    num: 49
    protection: '6.4'
    skills:
    - color: red
      description: Charged Hammer of Justice deals 2 bonus damage and inflicts Stun
        on secondary targets.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Cleave
      type: offense
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '57.1'
  - damage: '4.3'
    disable: '0.7'
    energy: '3.1'
    num: 24
    protection: '6.0'
    skills:
    - color: red
      description: Charged Hammer of Justice deals 2 bonus damage and inflicts Stun
        on secondary targets.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Cleave
      type: offense
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: pink
      description: Smite projectiles inflict a 35 Snare for 1.2.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Shackles
      type: control
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '54.2'
  - damage: '2.7'
    disable: '0.4'
    energy: '2.7'
    num: 18
    protection: '4.8'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: green
      description: Activating Smite grants a Shield. It absorbs up to 12 damage for
        3.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Halo
      type: survival
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '22.2'
  - damage: '4.6'
    disable: '0.7'
    energy: '3.5'
    num: 15
    protection: '4.9'
    skills:
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: teal
      description: Holy Light recharges 100 faster when Smite is active.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Faith
      type: support
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '33.3'
  - damage: '4.6'
    disable: '0.8'
    energy: '3.5'
    num: 14
    protection: '6.0'
    skills:
    - color: red
      description: Charged Hammer of Justice deals 2 bonus damage and inflicts Stun
        on secondary targets.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Cleave
      type: offense
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '42.9'
  - damage: '4.0'
    disable: '0.7'
    energy: '3.4'
    num: 14
    protection: '5.2'
    skills:
    - color: red
      description: Charged Hammer of Justice deals 2 bonus damage and inflicts Stun
        on secondary targets.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Cleave
      type: offense
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: grey
      description: Holy Light heals you for 8 health when another ally affected by
        Favor is healed.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Divine Abundance
      type: mixed
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '50.0'
  - damage: '5.7'
    disable: '0.8'
    energy: '4.1'
    num: 12
    protection: '5.0'
    skills:
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: red
      description: The third Smite attack deals 6 bonus damage and explodes upon impact
        dealing 16 area damage to nearby enemies.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Fire
      type: offense
    - color: yellow
      description: Intervene travel distance increases by 40 when affected by Favor.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Initiation
      type: mobility
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '91.7'
  - damage: '4.8'
    disable: '1.1'
    energy: '3.0'
    num: 11
    protection: '3.7'
    skills:
    - color: grey
      description: Holy Light heals you for 8 health when another ally affected by
        Favor is healed.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Divine Abundance
      type: mixed
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: pink
      description: Smite projectiles inflict a 35 Snare for 1.2.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Shackles
      type: control
    - color: yellow
      description: Absorbing attacks with Radiant Shield increases your movement speed
        by [Float 0.25,Float 0.45,Float 0.6] for 3.
      icon: c7f49424687c9a744a6347a5e2eede0b
      name: Light Speed
      type: mobility
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '54.5'
  - damage: '3.7'
    disable: '0.6'
    energy: '3.1'
    num: 10
    protection: '6.4'
    skills:
    - color: red
      description: Charged Hammer of Justice deals 2 bonus damage and inflicts Stun
        on secondary targets.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Cleave
      type: offense
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: pink
      description: Smite projectiles inflict a 35 Snare for 1.2.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Shackles
      type: control
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '70.0'
  twos:
  - damage: '6.3'
    disable: '1.1'
    energy: '3.4'
    num: 12
    protection: '5.1'
    skills:
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: green
      description: Activating Smite grants a Shield. It absorbs up to 12 damage for
        3.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Halo
      type: survival
    - color: pink
      description: Smite projectiles inflict a 35 Snare for 1.2.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Shackles
      type: control
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    winrate: '91.7'
  - damage: '4.6'
    disable: '0.7'
    energy: '3.0'
    num: 6
    protection: '4.1'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: red
      description: The third Smite attack deals 6 bonus damage and explodes upon impact
        dealing 16 area damage to nearby enemies.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Fire
      type: offense
    - color: pink
      description: Smite projectiles inflict a 35 Snare for 1.2.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Shackles
      type: control
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '83.3'
  - damage: '6.5'
    disable: '0.7'
    energy: '4.1'
    num: 5
    protection: '5.1'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: red
      description: The third Smite attack deals 6 bonus damage and explodes upon impact
        dealing 16 area damage to nearby enemies.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Fire
      type: offense
    - color: yellow
      description: Intervene travel distance increases by 40 when affected by Favor.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Initiation
      type: mobility
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '100.0'
  - damage: '5.5'
    disable: '0.8'
    energy: '3.8'
    num: 5
    protection: '5.5'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: grey
      description: Holy Light heals you for 8 health when another ally affected by
        Favor is healed.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Divine Abundance
      type: mixed
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: green
      description: Activating Smite grants a Shield. It absorbs up to 12 damage for
        3.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Halo
      type: survival
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '40.0'
  - damage: '6.4'
    disable: '0.8'
    energy: '3.9'
    num: 5
    protection: '4.1'
    skills:
    - color: red
      description: Hammer of Justice deals 3 bonus damage and the Stun duration of
        charged Hammer of Justice is increased by 0.2 when affected by Favor.
      icon: 398f1e921a73f484e9fc54e1308407f7
      name: Enchanted Weapon
      type: offense
    - color: red
      description: The third Smite attack deals 6 bonus damage and explodes upon impact
        dealing 16 area damage to nearby enemies.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Fire
      type: offense
    - color: yellow
      description: Intervene travel distance increases by 40 when affected by Favor.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Initiation
      type: mobility
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '100.0'
  - damage: '2.9'
    disable: '0.5'
    energy: '2.1'
    num: 4
    protection: '2.6'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: green
      description: Activating Smite grants a Shield. It absorbs up to 12 damage for
        3.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Halo
      type: survival
    - color: red
      description: The third Smite attack deals 6 bonus damage and explodes upon impact
        dealing 16 area damage to nearby enemies.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Fire
      type: offense
    - color: yellow
      description: Intervene travel distance increases by 40 when affected by Favor.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Initiation
      type: mobility
    - color: teal
      description: Blocking an attack with Radiant Shield sends out a wave of light,
        healing nearby allies for 12 health.
      icon: c7f49424687c9a744a6347a5e2eede0b
      name: Light's Embrace
      type: support
    winrate: '25.0'
  - damage: '5.9'
    disable: '0.9'
    energy: '3.5'
    num: 4
    protection: '4.5'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: teal
      description: Holy Light recharges 100 faster when Smite is active.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Faith
      type: support
    - color: red
      description: The third Smite attack deals 6 bonus damage and explodes upon impact
        dealing 16 area damage to nearby enemies.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Fire
      type: offense
    - color: pink
      description: Smite projectiles inflict a 35 Snare for 1.2.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Shackles
      type: control
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '100.0'
  - damage: '3.4'
    disable: '0.5'
    energy: '2.4'
    num: 3
    protection: '4.2'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: grey
      description: Holy Light heals you for 8 health when another ally affected by
        Favor is healed.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Divine Abundance
      type: mixed
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: green
      description: Activating Smite grants a Shield. It absorbs up to 12 damage for
        3.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Halo
      type: survival
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '0.0'
  - damage: '3.5'
    disable: '0.5'
    energy: '2.9'
    num: 2
    protection: '3.6'
    skills:
    - color: teal
      description: Holy Light healing increases by 3 when healing allies.
      icon: 85a79d3bf018b184887fb1a71ef51ac1
      name: Devotion
      type: support
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: green
      description: Activating Smite grants a Shield. It absorbs up to 12 damage for
        3.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Halo
      type: survival
    - color: red
      description: Intervene deals 3 bonus damage and reduces incoming healing by
        25 for 3.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Penitence
      type: offense
    - color: yellow
      description: Condemn cooldown is reduced by 3 and hits reduce the cooldown of
        Intervene by 4.
      icon: 5d4f9d83e84294c4e9412cb99c0e3e0c
      name: Vindicator
      type: mobility
    winrate: '0.0'
  - damage: '4.7'
    disable: '0.6'
    energy: '3.4'
    num: 2
    protection: '6.2'
    skills:
    - color: teal
      description: Applying Favor to an ally grants them a Shield. It absorbs up to
        12 damage for 1.5.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Divine Protection
      type: support
    - color: grey
      description: All effects of Divinity are doubled.
      icon: 96d5e8acf68061e478f9f6a8d315a4c4
      name: Godlike
      type: mixed
    - color: green
      description: Activating Smite grants a Shield. It absorbs up to 12 damage for
        3.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Halo
      type: survival
    - color: red
      description: The third Smite attack deals 6 bonus damage and explodes upon impact
        dealing 16 area damage to nearby enemies.
      icon: 6892bf3a45c0ad042b495a385cb5604f
      name: Holy Fire
      type: offense
    - color: yellow
      description: Intervene travel distance increases by 40 when affected by Favor.
      icon: 43ddc4074c2b605478c43c9727704ce8
      name: Initiation
      type: mobility
    winrate: '50.0'
compos:
  threes:
  - hero_names:
    - Ulric
    - Croak
    - Jade
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: d3038956f9767d74bbbce0659b07474d
      name: Croak
      title: croak
    - icon: 38b75268719e2b640a6d62250e102ffa
      name: Jade
      title: jade
    num: 26
    winrate: 0.7307692307692307
    wins: 19
  - hero_names:
    - Ulric
    - Bakko
    - Jade
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 6154d6c0556261343844bf7caebb67a7
      name: Bakko
      title: bakko
    - icon: 38b75268719e2b640a6d62250e102ffa
      name: Jade
      title: jade
    num: 24
    winrate: 0.4166666666666667
    wins: 10
  - hero_names:
    - Ulric
    - Thorn
    - Jumong
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    - icon: 42a665b9c285a1243a201c8d533980e5
      name: Jumong
      title: jumong
    num: 17
    winrate: 0.6470588235294118
    wins: 11
  - hero_names:
    - Ulric
    - Raigon
    - Jade
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 6c2252900d15cb344a5240f30d3ba705
      name: Raigon
      title: raigon
    - icon: 38b75268719e2b640a6d62250e102ffa
      name: Jade
      title: jade
    num: 15
    winrate: 0.4
    wins: 6
  - hero_names:
    - Ulric
    - Thorn
    - Varesh
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    - icon: f1cc53a29c58089478f6438589d9b32d
      name: Varesh
      title: varesh
    num: 13
    winrate: 0.5384615384615384
    wins: 7
  - hero_names:
    - Ulric
    - Ashka
    - Thorn
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 02fbe1fea9e22054a9ab1b0c5007dbae
      name: Ashka
      title: ashka
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    num: 13
    winrate: 0.46153846153846156
    wins: 6
  - hero_names:
    - Ulric
    - Raigon
    - Varesh
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 6c2252900d15cb344a5240f30d3ba705
      name: Raigon
      title: raigon
    - icon: f1cc53a29c58089478f6438589d9b32d
      name: Varesh
      title: varesh
    num: 13
    winrate: 0.5384615384615384
    wins: 7
  - hero_names:
    - Ulric
    - Jumong
    - Bakko
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 42a665b9c285a1243a201c8d533980e5
      name: Jumong
      title: jumong
    - icon: 6154d6c0556261343844bf7caebb67a7
      name: Bakko
      title: bakko
    num: 13
    winrate: 0.38461538461538464
    wins: 5
  - hero_names:
    - Ulric
    - Rook
    - Jade
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    - icon: 38b75268719e2b640a6d62250e102ffa
      name: Jade
      title: jade
    num: 12
    winrate: 0.75
    wins: 9
  - hero_names:
    - Ulric
    - Thorn
    - Raigon
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    - icon: 6c2252900d15cb344a5240f30d3ba705
      name: Raigon
      title: raigon
    num: 12
    winrate: 0.75
    wins: 9
  twos:
  - hero_names:
    - Ulric
    - Rook
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: e5d5913527b842b41aed666d27d14231
      name: Rook
      title: rook
    num: 24
    winrate: 0.7083333333333334
    wins: 17
  - hero_names:
    - Ulric
    - Bakko
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 6154d6c0556261343844bf7caebb67a7
      name: Bakko
      title: bakko
    num: 16
    winrate: 0.6875
    wins: 11
  - hero_names:
    - Ulric
    - Raigon
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 6c2252900d15cb344a5240f30d3ba705
      name: Raigon
      title: raigon
    num: 15
    winrate: 0.6666666666666666
    wins: 10
  - hero_names:
    - Ulric
    - Freya
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: be081702a8e70fe44ac0a4e3a9c46368
      name: Freya
      title: freya
    num: 10
    winrate: 0.8
    wins: 8
  - hero_names:
    - Ulric
    - Croak
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: d3038956f9767d74bbbce0659b07474d
      name: Croak
      title: croak
    num: 6
    winrate: 0.5
    wins: 3
  - hero_names:
    - Ulric
    - Thorn
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: aab126231d9ff344f9f88ab230a5e66f
      name: Thorn
      title: thorn
    num: 6
    winrate: 0.5
    wins: 3
  - hero_names:
    - Ulric
    - Shifu
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 6f69edc7410650c43bee267eef585b06
      name: Shifu
      title: shifu
    num: 5
    winrate: 0.4
    wins: 2
  - hero_names:
    - Ulric
    - Destiny
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: 65e28ab3b75c7cb478a1b24b46fca7d2
      name: Destiny
      title: destiny
    num: 4
    winrate: 0.25
    wins: 1
  - hero_names:
    - Ulric
    - Ruh Kaan
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: bb37e5d448c511e47b808987d0d4f9e8
      name: Ruh Kaan
      title: ruh-kaan
    num: 4
    winrate: 0.0
    wins: 0
  - hero_names:
    - Ulric
    - Iva
    heros:
    - icon: a90600a0f93cf3644b279b61ab3dc5c8
      name: Ulric
      title: ulric
    - icon: decba44679fbdae4bb7d0d1cb6a97682
      name: Iva
      title: iva
    num: 4
    winrate: 0.0
    wins: 0
description: Sworn protector of the weak, devout follower of the Way of Symphony,
  and unwavering light against the dark. Ulric, trained in the holy Monastery of Resonance,
  has spent many years traveling the lands to spread the word of the Light. He joins
  the Arena now to demonstrate to allies, foes, and onlookers alike the primacy of
  the True Path.
icon: a90600a0f93cf3644b279b61ab3dc5c8
layout: character
name: ulric
num:
  threes: 700
  twos: 114
title: Ulric
url: characters/ulric.html
winrate:
  threes: '47.14'
  twos: '53.51'
...
